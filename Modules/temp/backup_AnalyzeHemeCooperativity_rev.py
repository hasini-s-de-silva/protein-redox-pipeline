...
def plot_potential_progression(energy_history, filename):
    ...
    colors = plt.cm.rainbow(np.linspace(0, 1, num_hemes))
...
def plot_DG_landscape(directory, delta_G_ind, delta_G_seq_all, filename):
    ...
    colors = list(plt.cm.Greys(np.linspace(0.8, 0.2, num_sequential)))
    
    for i, delta_G_seq in enumerate(delta_G_seq_all):
        y_seq = [dg[4] for dg in delta_G_seq]
        plt.plot(x, y_seq, 'k:', linewidth=1, zorder=1)
        plt.scatter(x, y_seq, marker='s', s=50, facecolors=colors[i], edgecolors='black', 
                   linewidth=1, zorder=2, label=f'Seq{i+1}')
    
    y_ind = [dg[4] for dg in delta_G_ind]
    plt.plot(x, y_ind, 'k:', linewidth=1, zorder=3)
    plt.scatter(x, y_ind, marker='s', s=50, facecolors='white', edgecolors='black', 
               linewidth=1, zorder=4, label='Ind.')
...
def calculate_fractions(E_range, potentials, source_name):
    ...
    if 'independent' in potentials:
        ...
        data[f'F_ox ({source_name} Ind)'] = [calculate_f_ox_independent(E, ind_values) for E in E_range]
        data[f'F_red ({source_name} Ind)'] = [calculate_f_red_independent(E, ind_values) for E in E_range]
    
    if 'sequential' in potentials:
        for i, seq_potentials in enumerate(potentials['sequential']):
            seq_values = list(seq_potentials.values())
            data[f'F_ox ({source_name} Seq {i+1})'] = [calculate_f_ox_sequential(E, seq_values) for E in E_range]
            data[f'F_red ({source_name} Seq {i+1})'] = [calculate_f_red_sequential(E, seq_values) for E in E_range]
            ...
            if (plot_option == 'ox' and 'ox' in label.lower()) or \
               ...
               plot_option == 'both':
                ...
                if 'Ind' in label:
                    ...
                    # Extract sequential number safely
                    seq_num = 1  # default value
                    for part in label.split():
                        if part.startswith('Seq'):
                            try:
                                ...
                                pass
                    color = plt.cm.Greys(0.8 - 0.2 * seq_num)
                    linestyle = '-'
