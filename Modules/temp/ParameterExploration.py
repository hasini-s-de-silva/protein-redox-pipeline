...
def main():
    ...
    # New parameters for target D sampling
    ...
    while accepted < num_sets and attempts < max_attempts:
        ...
        # If computation failed or D is None, skip this set
        if D is None:
            continue
            
        # Check if D is within tolerance of target (if targeting)
        if d_target > 0:
            relative_error = abs(D - d_target) / d_target
