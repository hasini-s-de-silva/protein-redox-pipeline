...
class RedoxCurrentAnalyzer:
    ...
    def _create_iv_plot(self, voltages: np.ndarray, currents_dict: Dict[str, list]) -> None:
        """Create current vs voltage plot with proper z-order"""
        fig, ax1 = plt.subplots(figsize=(3.3, 3.3))
    
        # Colors
        purple_color = '#800080'  # for flux-derived
        dark_orange = '#FF8C00'  # darker orange for diffusion-derived
    
        # First plot flux-derived current (with lower z-order)
        ax1.set_xlabel('Voltage (V)')
        if 'Flux' in currents_dict:
            ax2 = ax1.twinx()
            ax2.plot(voltages, np.array(currents_dict['Flux']) * 1e3,  # convert pA to fA
                    color=purple_color, linestyle='-', zorder=1)
            ax2.set_ylabel('Flux-derived Current (fA)', color=purple_color)
            ax2.tick_params(axis='y', labelcolor=purple_color, direction='in')
            ax2.yaxis.set_minor_locator(AutoMinorLocator())
            ax2.tick_params(which='minor', direction='in')
    
        # Then plot diffusion-derived current (with higher z-order)
        if 'Diffusion' in currents_dict:
            # Draw the line with higher z-order and slightly thicker
            ax1.plot(voltages, np.array(currents_dict['Diffusion']) * 1e3,  # convert pA to fA
                    color=dark_orange, linestyle='--', linewidth=2, zorder=10)
            ax1.set_ylabel('Diffusion-derived Current (fA)', color=dark_orange)
            ax1.tick_params(axis='y', labelcolor=dark_orange, direction='in')
            ax1.tick_params(axis='x', direction='in')
            ax1.xaxis.set_minor_locator(AutoMinorLocator())
            ax1.yaxis.set_minor_locator(AutoMinorLocator())
            ax1.tick_params(which='minor', direction='in')
    
        # If experimental data exists (with highest z-order)
        if 'Experimental' in currents_dict:
            ax3 = ax1.twinx()
            ax3.spines['right'].set_position(('outward', 60))
            ax3.plot(voltages, currents_dict['Experimental'], 
                    color='black', linestyle='-', zorder=3)
            ax3.set_ylabel('Experimental Current (pA)', color='black')
            ax3.tick_params(axis='y', labelcolor='black', direction='in')
