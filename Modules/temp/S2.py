...
def worker_process(args):
    ...
    sequence, free_eng_opt, spacing_factor, d_target, d_tolerance, progress_tracker, shared_results = args
    ...
    while True:
        ...
        progress_tracker.increment_attempts()
        ...
        if d_target > 0:
            ...
            if relative_error > d_tolerance:
                if progress_tracker.should_print_progress():
        ...
        with progress_tracker.lock:
            progress_tracker.increment_accepted()
            shared_results.append((D_scaled, row_data))
            if progress_tracker.should_print_progress():
                progress_tracker.print_progress()
            
            if progress_tracker.accepted.value >= progress_tracker.total_sets:
                return
...
def run_parallel_sampling(num_processes, num_sets, sequence, free_eng_opt, 
    ...
    with Manager() as manager:
        ...
        progress_tracker = ProgressTracker(num_sets)
        
        # Create arguments for each worker
        worker_args = (sequence, free_eng_opt, spacing_factor, 
                      d_target, d_tolerance, progress_tracker, shared_results)
        
        # Create pool and run workers
        with Pool(num_processes) as pool:
            # Create multiple workers with the same arguments
            args_list = [worker_args] * num_processes
        ...
        return results, progress_tracker.attempts.value
...
def main():
    ...
    accept_rate = (len(results) / total_attempts * 100 if total_attempts > 0 else 0)
    print(f"\nFinal Statistics:")
    print(f"Total attempts: {total_attempts:,d}")  # Use thousands separator
    ...
    if len(results) < num_sets:
        print(f"\nWarning: Only found {len(results)} sets out of {num_sets} requested")
        if d_target > 0:
            print(f"Consider adjusting d_target ({d_target:.2e}) or tolerance ({d_tolerance:.2f})")

    # Sort results by diffusion coefficient (already sorted, but ensure correct order)
