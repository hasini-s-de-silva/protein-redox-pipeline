...
def write_log_header(log_file, case, direction):
    ...
    case: str - The calculation case ('reduced', 'oxidized', or 'mixed')
    direction: str - The calculation direction ('forward' or 'reverse')
    """
    log_file.write(f"\n{'='*80}\n")
    log_file.write(f"Case: {case}, Direction: {direction}\n")
    log_file.write(f"{'='*80}\n\n")
    log_file.write("Iteration  Max_Change(meV)  Flux(10⁶ s⁻¹)  Mix_Param  Redox_Potentials(eV)  Populations\n")
    log_file.write("-" * 100 + "\n")

def write_log_entry(log_file, iteration, max_diff, flux, redox_potentials, populations, mix_param):
    """
    ...
    max_diff: float - Maximum change in potentials from previous iteration
    flux: float - Current flux value
    redox_potentials: array-like - Current redox potentials
    populations: array-like - Current populations
    mix_param: float - Current mixing parameter value
    """
    ...
    log_file.write(f"{iteration:9d}  {max_diff:13.6f}  {flux/1e6:13.6f}  {mix_param:9.3f}  {potentials_str}  {populations_str}\n")
...
def adaptive_mixing_parameter(iteration, max_diff, prev_max_diff, current_param,
                            max_diff_history=None, window_size=3):
    """
    Adaptively adjust the mixing parameter based on convergence behavior with oscillation detection.
    ...
    max_diff_history: list storing recent max_diff values for oscillation detection
    window_size: number of previous iterations to consider for oscillation detection

    Returns:
    tuple: (new_mixing_parameter, updated_history)
    """
    ...
    # Detect oscillations by checking if values alternate
    ...
    if is_oscillating:
        # More aggressive reduction if oscillating
        new_param = max(0.1, current_param * 0.7)
    elif max_diff > prev_max_diff:
        # Normal reduction if error increasing
        new_param = max(0.1, current_param * 0.9)
    else:
        # Gentle increase if converging well
        new_param = min(0.8, current_param * 1.05)
...
def main():
    ...
    parser.add_argument('--adaptive-mixing', action='store_true',
                        help='Enable adaptive mixing parameter adjustment')
    ...
    with open(log_txt, 'w') as log_file:
        ...
        log_file.write(f"Initial mixing parameter: {args.mixing_param}\n")
        log_file.write(f"Adaptive mixing: {'enabled' if args.adaptive_mixing else 'disabled'}\n\n")
        ...
        for case in ['reduced', 'oxidized', 'mixed']:
            ...
            for direction in ['forward', 'reverse']:
                ...
                mixing_param = args.mixing_param
                ...
                while True:
                    ...
                    # Calculate new redox potentials
                    R_new = compute_redox_potentials(E, case, populations)
                    if direction == 'reverse':
                        R_new = R_new[::-1]

                    # Calculate convergence before mixing
                    max_diff = calculate_max_difference(R_new, prev_R)

                    # Apply mixing with current mixing parameter
                    R = mixing_param * R_new + (1 - mixing_param) * prev_R

                    # Update mixing parameter if adaptive mixing is enabled
                    if args.adaptive_mixing and iteration > 0:
                        mixing_param, max_diff_history = adaptive_mixing_parameter(
                            iteration, max_diff, prev_max_diff, mixing_param, max_diff_history)

                    # Write log entry
                    write_log_entry(log_file, iteration, max_diff, flux, R, populations, mixing_param)

                    if max_diff < threshold:
                        print(f"    Converged after {iteration+1} iterations!")
                        print(f"    Final maximum change: {max_diff:.6f} meV")
                        print(f"    Final mixing parameter: {mixing_param:.3f}")
                        break
